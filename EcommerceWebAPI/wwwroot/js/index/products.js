import { apiUrl, formatNumber, normalizeProduct } from './utils.js';
import { addItemToGuestCart, renderCartFromStorage } from './cart.js';

let PRODUCTS_CACHE = [];
let _els = { productsGrid: null, searchInput: null, stockOnlySwitch: null };

export async function fetchProductsLowStock() {
  const url = apiUrl('/api/productos?activo=1'); // sin &maxStock=1 (según tu original)
...
function makeProductCard(p) {
  ...
  // Overlays de stock / ¡Último! centrado con opacidad 40%
  if (stock <= 0) {
    ...
    Object.assign(overlayLow.style, {
      ...
      backgroundColor: 'rgba(255,165,0,0.4)', // 40%
      color: '#ffffffff', border: '1px solid rgba(255,165,0,0.7)',
      borderRadius: '10px', fontWeight: '600', zIndex: '2', pointerEvents: 'none'
    });
    media.appendChild(overlayLow);
  }

  // Agregar al carrito con tope por stock
