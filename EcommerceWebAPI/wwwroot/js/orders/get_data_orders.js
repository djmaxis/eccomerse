...
(function () {
  ...
  function normalizePayload(raw) {
    ...
    out.ordenes = list.map(o => {
      ...
      const idCliente = get(o, 'IdCliente', 'idCliente');
      const estado = get(o, 'Estado', 'estado');
      const trackingNumber = get(o, 'TrackingNumber', 'trackingNumber') || '';
      ...
      const factura = facturaSrc ? {
        ...
        NumeroFactura: get(facturaSrc, 'NumeroFactura', 'numeroFactura') ?? null
            ...
            // Dentro de out.ordenes = list.map(o => { ... return { ... }; });
      ...
      const metodoPago = mpSrc ? {
        ...
        NumeroTarjeta: get(mpSrc, 'NumeroTarjeta', 'numeroTarjeta'),
        cvv: get(mpSrc, 'cvv', 'CVV', 'Cvv'),
...
(function(){
  ...
  function makeOrderCard({ IdOrden, FechaCreacion, Productos, Pago }) {
    ...
    badge.className = 'badge-dot status in-transit';
    badge.textContent = 'En tránsito'; // #2 Forzado en UI
...
async function renderOrders(json) {
  ...
  async function init() {
    ...
    const json = normalizePayload(r.data);
    // Guardado opcional del JSON (espejo)
    await saveJsonToServer(json).catch(() => {});

    // Render dinámico
    await renderOrders(json);

    // Evento para otros listeners
